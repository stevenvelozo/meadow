...
class MeadowProviderALASQL extends libMeadowProviderBase
{
	constructor(pMeadow)
	{
		...
		if (!_Fable.hasOwnProperty(`ALASQL`))
		{
			...
			this.Meadow.log.fatal('Meadow is trying to perform queries without a valid [Fable.ALASQL] object.  See the documentation for how to initialize one.');
			return false;
		}

		this.libALASQL = _Fable.ALASQL;

		this._Scope = 'Unknown_Meadow_ALASQL_Scope';
		this._Schema = {};
		this._DefaultIdentifier = 'ID';
		this._DefaultGUIDentifier = 'GUID';
	}
	...
	{
		let tmpCreateStatement = '';
		let tmpTable = _Scope;
		let tmpSchema = _Schema;
		...
		this.Meadow.log.info('Auto Creating ALASQL database `'+tmpTable+'`', {CreateStatement:tmpCreateStatement});
	...
	Create(pQuery, fCallback)
	{
		...
		let fQuery = libALASQL.compile(pQuery.query.body);
		...
		try
		{
			...
			let tmpQueryResponse = fQuery(pQuery.query.parameters);
	...
	Undelete(pQuery, fCallback)
	{
		...
		pQuery.setDialect(this._Dialect).buildUndeleteQuery();
		let fQuery = libALASQL.compile(pQuery.query.body);
	 ...
	 * }
	 */
	constructFromObject(pParameters)
	{
		...
		// Construct a meadow
		let tmpMeadow = pParameters.Meadow
		...
		for (let tmpProperty in pParameters.ObjectPrototype)
		{
			let tmpAdded = false;
		...
		{
			for (let j = 0; j < pParameters.Data.length; j++)
			{
				tmpMeadow.doCreate(tmpMeadow.query.clone().addRecord(pParameters.Data[j]),
						function(pError, pQuery, pQueryRead, pRecord)
						{
							// Maybe log the error?
							this.Meadow.log.trace('Auto imported record', pRecord);
						}
					);
			}
		}
